    ...
    export default {
        ...
        computed: {
            ...
            releventShipmentSchedule: function() {
                const list = this.shipmentSchedule.filter((shipment) => {
        ...
        methods: {
            ...
            generateSqno: function(reservationType) {
                const contractType = this.workingSupplierDetail.contractType;
                const CUS_NO = this.workingSupplierDetail.CUS_NO;
                const yearPart = (this.workingYear - 1911).toString();
                const monthPart = ('00' + this.workingMonth).slice(-2);
                const oneTimeDatePart = this.cellDateString.slice(-10, 4) + moment.utc(new Date(this.cellDateString)).format('MMDD');
